...
export default async function DashboardPage() {
  try {
    ...
    // Separate queries to avoid type issues
    let orders: any[] = []
    if (isAdmin) {
      const { data } = await supabase
        .from('orders')
        .select('id, title, paper_type, pages, deadline, price, status, payment_status, created_at, user_id')
      ...
      const { data } = await supabase
        .from('orders')
        .select('id, title, paper_type, pages, deadline, price, status, payment_status, created_at')
        .eq('user_id', user.id)
        .order('created_at', { ascending: false })
      orders = data || []
    }
      ...
      <>
        ...
        <div style={{ minHeight: '100vh', background: '#080f1e', fontFamily: 'Outfit, sans-serif' }}>
          <div style={{ maxWidth: 1100, margin: '0 auto', padding: '2.5rem 1.5rem' }}>
            ...
            {/* ── ADMIN BANNER ── only shows for admin */}
            {isAdmin && (
              <div style={{ background: 'rgba(0,229,255,0.06)', border: '1px solid rgba(0,229,255,0.25)', borderRadius: 16, padding: '1.25rem 1.5rem', marginBottom: '1.75rem', display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: '1rem' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
                  <div style={{ width: 38, height: 38, borderRadius: 8, background: 'linear-gradient(135deg,#00E5FF,#00ff87)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '1.1rem', flexShrink: 0 }}>⚡</div>
                  <div>
                    <div style={{ fontWeight: 800, color: '#eef2f7', fontSize: '0.95rem' }}>Admin Account</div>
                    <div style={{ color: '#8892a4', fontSize: '0.8rem' }}>You have full access to manage orders, users and file delivery</div>
                  </div>
                </div>
                <Link href="/admin" style={{ background: 'linear-gradient(135deg,#00E5FF,#00b8cc)', color: '#080f1e', padding: '0.7rem 1.5rem', borderRadius: 10, fontWeight: 800, fontSize: '0.9rem', textDecoration: 'none', whiteSpace: 'nowrap' }}>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '2rem', flexWrap: 'wrap', gap: '1rem' }}>
              <div>
                <h1 style={{ fontSize: '1.8rem', fontWeight: 900, letterSpacing: '-0.03em', marginBottom: '0.25rem', color: '#eef2f7' }}>
                  Welcome back,{' '}
                  <span style={{ background: 'linear-gradient(135deg,#00E5FF,#00ff87)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent' }}>
                    {firstName}
                  </span>
                </h1>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(180px, 1fr))', gap: '1rem', marginBottom: '2rem' }}>
              {[
                { label: isAdmin ? 'Recent Orders' : 'Total Orders', value: stats.total, color: '#8892a4', icon: '📋' },
            ...
            <div style={{ background: '#0e1a2e', border: '1px solid rgba(0,229,255,0.1)', borderRadius: 16, overflow: 'hidden' }}>
              <div style={{ padding: '1.1rem 1.5rem', borderBottom: '1px solid rgba(0,229,255,0.08)', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                <span style={{ fontWeight: 700, color: '#eef2f7', fontSize: '0.95rem' }}>
                  {isAdmin ? 'Latest 10 Orders — All Students' : 'My Orders'}
                </span>
                {isAdmin && (
              ...
              {orders.length === 0 ? (
                <div style={{ padding: '4rem', textAlign: 'center' }}>
                  ...
                  <div style={{ color: '#8892a4', marginBottom: '1rem', fontFamily: 'Outfit, sans-serif' }}>No orders yet</div>
                        ...
                        return (
                          <tr key={o.id} style={{ borderBottom: '1px solid rgba(0,229,255,0.05)' }}>
                            <td style={{ padding: '1rem 1.25rem', maxWidth: 200 }}>
                              <div style={{ fontSize: '0.85rem', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', color: '#eef2f7', fontFamily: 'Outfit, sans-serif' }}>{o.title}</div>
                            </td>
                            <td style={{ padding: '1rem 1.25rem', color: '#8892a4', fontSize: '0.82rem', whiteSpace: 'nowrap', fontFamily: 'Outfit, sans-serif' }}>{o.paper_type}</td>
                            <td style={{ padding: '1rem 1.25rem', color: '#8892a4', fontSize: '0.82rem', fontFamily: 'Outfit, sans-serif' }}>{o.pages}</td>
                            <td style={{ padding: '1rem 1.25rem', color: '#8892a4', fontSize: '0.82rem', whiteSpace: 'nowrap', fontFamily: 'Outfit, sans-serif' }}>{new Date(o.deadline).toLocaleDateString()}</td>
                            <td style={{ padding: '1rem 1.25rem', color: '#00E5FF', fontWeight: 700, fontSize: '0.88rem', fontFamily: 'Outfit, sans-serif' }}>${o.price}</td>
                            <td style={{ padding: '1rem 1.25rem' }}>
                              <span style={{ background: sc.bg, color: sc.color, borderRadius: 100, padding: '0.2rem 0.7rem', fontSize: '0.72rem', fontWeight: 700, whiteSpace: 'nowrap', fontFamily: 'Outfit, sans-serif' }}>
                                {sc.label}
                              </span>
                            </td>
                          </tr>
              ...
              <div style={{ marginTop: '1.5rem', display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '1rem' }}>
                {[
                  ...
                  <Link key={item.label} href={item.href} style={{ background: '#0e1a2e', border: '1px solid rgba(0,229,255,0.1)', borderRadius: 12, padding: '1.1rem', textDecoration: 'none', display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
                    ...
                    <div>
                      <div style={{ fontWeight: 700, color: '#00E5FF', fontSize: '0.85rem', fontFamily: 'Outfit, sans-serif' }}>{item.label}</div>
                      <div style={{ color: '#8892a4', fontSize: '0.75rem', fontFamily: 'Outfit, sans-serif' }}>{item.desc}</div>
                    </div>
                  </Link>
                ))}
              </div>
