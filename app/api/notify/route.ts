...
export async function POST(req: NextRequest) {
  try {
    ...
    const clientName = profile?.full_name || 'Student'

    switch (type) {
      case 'new_order':
        await sendNewOrderAdmin({ ...order, user_email: clientEmail, user_name: clientName })
        if (clientEmail) await sendOrderConfirmation(order, clientEmail, clientName)
        break
      case 'status_update':
        if (clientEmail) await sendStatusUpdate(order, clientEmail, clientName)
        break
